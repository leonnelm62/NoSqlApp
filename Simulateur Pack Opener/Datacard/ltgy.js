class Card {
    constructor(element) {
        ...
        for (let set in element.card_sets) {
            if (element.card_sets[set].set_name === "Lord of the Tachyon Galaxy") {
...
function cardsData() { // Récupération de données d'API
    fetch('https://db.ygoprodeck.com/api/v7/cardinfo.php?cardset=Lord%20of%20the%20Tachyon%20Galaxy')
...
function boosterChance(set) {
    ...
    boosterOpened++
    if (boosterOpened == 2 || boosterOpened == 5 || boosterOpened == 10) 
        makeSuperBooster(set);
    
    else if (boosterOpened == 4 || boosterOpened == 8)
...
function trunk(booster) {
 
    var trunk = JSON.stringify(booster)
    var fs = require('fs')
    fs.writeFile("loot.json", trunk)
